...
let WIDTH = window.innerWidth,
  ...
  PROJECTILEDAMAGE = 20
// Global vars
let t = THREE, scene, cam, renderer, controls, clock, projector, model, skin
let runAnim = true, mouse = { x: 0, y: 0 }, kills = 0, health = 100
let healthCube, lastHealthPickup = 0
/*
...
function setupScene() {
  ...
  // Geometry: walls
  const cube = new t.CubeGeometry(UNITSIZE, WALLHEIGHT, UNITSIZE)
  const materials = [
  ...
  for (let i = 0; i < mapW; i++) 
    for (let j = 0, m = map[i].length; j < m; j++) 
      if (map[i][j]) {
        const wall = new t.Mesh(cube, materials[map[i][j] - 1])
...
var ai = []
const aiGeo = new t.CubeGeometry(40, 40, 40)
function setupAI() {
...
function addAI() {
  ...
  const aiMaterial = new t.MeshBasicMaterial({/* color: 0xEE3333,*/map: t.ImageUtils.loadTexture('images/face.png')})
  const o = new t.Mesh(aiGeo, aiMaterial)
  do {
...
function getAIpath(a) {
  const p = getMapSector(a.position)
  do { // Cop-out
    do {
      var x = getRandBetween(0, mapW - 1)
      var z = getRandBetween(0, mapH - 1)
...
function findAIpath(sX, sZ, eX, eZ) {
  ...
  const path = finder.findPath(sX, sZ, eX, eZ, grid)
  grid = backupGrid
  return path
}
...
// Radar
function drawRadar() {
  const c = getMapSector(cam.position), context = document.getElementById('radar').getContext('2d')
...
var bullets = []
const sphereMaterial = new t.MeshBasicMaterial({color: 0x333333})
const sphereGeo = new t.SphereGeometry(2, 6, 6)
function createBullet(obj) {
  if (obj === undefined) 
    obj = cam
	
  const sphere = new t.Mesh(sphereGeo, sphereMaterial)
