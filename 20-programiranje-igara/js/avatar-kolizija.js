...
function praviDrvo(x, z) {
  ...
  var granica = new THREE.Mesh(new THREE.CircleGeometry(200), new THREE.MeshNormalMaterial());
...
function detektujSudare() {
  var vektor = new THREE.Vector3(0, -1, 0);
  var zrak = new THREE.Ray(marker.position, vektor);
  var ukrstanja = zrak.intersectObjects(zabranjeni_prilazi);
  if (ukrstanja.length > 0)
    return true;
  return false;
}
